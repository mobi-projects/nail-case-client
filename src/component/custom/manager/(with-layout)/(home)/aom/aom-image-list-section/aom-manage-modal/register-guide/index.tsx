...
export function RegisterGuide({ isGuideVisible }: RegisterGuidePT) {
	...
	return (
		<ModalBody
			className={cn(
				"absolute left-0 top-0 flex h-full transform flex-col justify-evenly gap-y-2 pt-5",
				slideCss,
			)}
		>
			<div>
				<p className="pb-2 text-Title02 font-SemiBold">
				...
				<div className="flex items-center text-Caption01 text-Gray70">
					...
					에서는 사장님들의 편리한 예약을 도와드리기 위해 이달의 아트를 등록 할
					때<span className="px-1 text-red-500"> 주의</span> 해야할 점이 있어요.
				</div>
			</div>
			<div>
				<p className="pb-2 text-Title02 font-SemiBold">등록 방법</p>
